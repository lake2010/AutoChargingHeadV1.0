...
void CLED_setup( CLED_parameter* CLED_para, Gpio_pin_parameter* Gpio_pin_para )
{
	...
	CLED_para->m_chargeLedBlink = true;
...
void CChargeBaseLED_loop( CLED_parameter* CLED_para )
{
	...
	if(CLED_para->m_ledIsOn){
		if((millis() - CLED_para->m_ledOnTime) > 300){
			...
			CLED_para->m_chargeLedBlink = false;
		}   
	}
	else if(CLED_para->m_chargeLedBlink && (millis() - CLED_para->m_ledOnTime) > 600)
	{
		CLED_para->m_ledOnTime = millis();
